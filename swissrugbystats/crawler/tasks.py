...
def update_all(deep_crawl=True, season=config.CURRENT_SEASON):
    ...
    crawler.crawl_teams([(c.league.shortCode, c.get_league_url(), c.id) for c in Competition.objects.filter(season=s)])

    # update game table with fixtures
    print("current season:" + config.CURRENT_SEASON)
    #crawler.crawl_fixtures_async([(c.league.shortCode, c.get_fixtures_url()) for c in Competition.objects.filter(season=config.CURRENT_SEASON)], deep_crawl)
    crawler.crawl_fixtures([(c.league.shortCode, c.get_fixtures_url(), c.id) for c in Competition.objects.filter(season=s)], deep_crawl)

    # update game table with results
    #crawler.crawl_results_async([(c.league.shortCode, c.get_results_url()) for c in Competition.objects.filter(season=config.CURRENT_SEASON)], deep_crawl)
    crawler.crawl_results([(c.league.shortCode, c.get_results_url(), c.id) for c in Competition.objects.filter(season=s)], deep_crawl)
